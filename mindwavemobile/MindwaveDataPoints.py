...
class EEGPowersDataPoint(DataPoint):
    def __init__(self, dataValueBytes):
        ...
        self.maxint = 2**24 #3 byte integers
    ...
    def asDict(self):
        return {"delta":self.delta, "theta":self.theta, "lowAlpha":self.lowAlpha, "highAlpha":self.highAlpha, 
                    "lowBeta":self.lowBeta, "highBeta":self.highBeta, "lowGamma":self.lowGamma, "midGamma":self.midGamma}
    def asList(self):
        return [self.delta, self.theta, self.lowAlpha,self.highAlpha,self.lowBeta,self.highBeta,self.lowGamma,self.midGamma]                
